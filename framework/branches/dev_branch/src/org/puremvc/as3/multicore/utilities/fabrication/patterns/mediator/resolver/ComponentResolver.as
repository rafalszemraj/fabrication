...
package org.puremvc.as3.multicore.utilities.fabrication.patterns.mediator.resolver
{
    ...
    dynamic public class ComponentResolver extends Proxy implements IEventDispatcher, IDisposable
    {
        ...
        private function calcComponentFromPath(path:String):UIComponent
        {
            ...
            for (var i:int = 0; i < n; i++) {
                ...
                if (regres != null) {
                    ...
                    if (currentIDInt < getChildrenCount(currentComponent)) {
                        if (currentComponent.hasOwnProperty("numElements"))
                            currentComponent = currentComponent["getElementAt"](currentID) as UIComponent;
                        else
        ...
        private function findComponentByID(component:UIComponent, id:String):UIComponent
        {
            ...
            var n:int = getChildrenCount(component);
            var child:UIComponent;

            for (var i:int = 0; i < n; i++) {

                if (component.hasOwnProperty("numElements"))
                    child = component["getElementAt"](i) as UIComponent;
                else
                    child = component.getChildAt(i) as UIComponent;
        ...
        private function findComponentChildByName(currentComponent:UIComponent, name:String):UIComponent
        {
            ...
            if (currentComponent.hasOwnProperty(name))
                ...
                if (!hasFound) {
                    numChildren = currentComponent.numChildren;
                    for (i = 0; i < numChildren; ++i) {
                        child = currentComponent.getChildAt(i) as UIComponent;

                        if (getChildrenCount(child) > 0)
                            return findComponentChildByName(child, name);
                    }
                }
        ...
        private function getChildrenCount(component:UIComponent):int
        {
            if (!component)
                return 0;

            if (component.hasOwnProperty("numElements"))
                return component["numElements"];
            else
                return component.numChildren;
        }
